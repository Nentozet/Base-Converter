...
***Обязательные*** **аргументы:**
   - ```type=<номер типа задания>``` - тип задания.
     ...
     | 3 | Найдите значение выражения и запишите его в системе счисления ```<случайное целое число в диапазоне [2, 36]>```: ```<случайное целое число>``` ```<случайный знак действия (+, -, *, /)>``` ```<случайное целое число>``` |
